...
//  console.log(_id)
  ...
  <img className="w-full " src={image} alt={name} />
      ...
      <div className="p-6">
        <h2 className="text-2xl font-semibold text-gray-800 dark:text-white">{name.slice(0,20)}</h2>
        <p className="text-sm font-medium text-gray-600 dark:text-gray-400">{authorName}</p>
      
      </div>
      <Link to={`/details/${_id}`} className="flex justify1 px-6 pb-6">
        <button  className="px-4 py-2 bg-blue-500 text-white font-semibold rounded-md hover:bg-blue-600 focus:outline-none focus:bg-blue-600">
          Details
        </button>
      </Link>
    </div>
